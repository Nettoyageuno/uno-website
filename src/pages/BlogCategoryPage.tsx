...
import { getPostsByCategory, getAllCategories, formatDate } from '../data/posts';
...
const BlogCategoryPage: React.FC = () => {
  ...
  const categoryName = categories.find(c => slugify(c) === category);
  ...
  const getCategoryColor = (category: string): 'emerald' | 'blue' | 'indigo' | 'violet' | 'amber' | 'teal' | 'gray' => {
  ...
  return (
    <div className="bg-white">
      <SEO 
        title={`${categoryName} Articles | Nettoyage Uno Blog`}
        description={`Expert insights and tips about ${categoryName.toLowerCase()} for Montréal businesses. Professional guidance from commercial cleaning specialists.`}
        keywords={`${categoryName.toLowerCase()}, commercial cleaning, Montréal business resources, facility management`}
      />
          ...
          <FadeInSection>
            ...
            <div className="text-center">
              <div className="mb-4">
                <CategoryPill 
                  label={categoryName}
                  icon={categoryName?.toLowerCase() === 'sustainability' ? <Leaf className="w-4 h-4" /> : undefined}
                ...
                <div className="space-y-2">
                  {categories.filter(c => c !== categoryName).map((cat) => (
                    <Link
                      ...
                      className="block text-gray-600 hover:text-gray-900 text-sm transition-colors focus:outline-none focus:ring-2 focus:ring-gray-900 focus:ring-offset-2 rounded"
                    >
                      {cat}
                    </Link>
                  ))}
